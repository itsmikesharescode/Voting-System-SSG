    ...
    if (data[0].voting_active) {
        if (user) {
            ...
            return {
                ...
                votedCandidates: await supabaseAdmin.from("vote_records_tb").select("*").eq("user_id", user.id).order("created_at", { ascending: true }) as PostgrestSingleResponse<VotedCandidatesDB[]>
            }
