...
export const actions: Actions = {
    ...
    //votes route
    printAction: async ({ locals: { supabaseAdmin }, url, request }) => {

        const formData = await request.formData();
        const classification = formData.get("classification") as string;

        const pdfPath = path.join(process.cwd(), 'example.pdf');

        const customCandidateArray = (tempArray: CandidatesDB[] | null) => {
        ...
        const generateHTMLTemplate = (data: PrintingType[] | undefined) => {
            let html = '';

            // Iterate over each item in the array
            data?.forEach(item => {
                // Start building HTML for each running position
                html += `<div class="position-container">`;
                html += `<h2>${item.runningPosition}</h2>`;
                html += `<p style="font-weight: bold;">Maximum Votes: ${item.maxVote}</p>`;

                // Start the grid layout for candidates
                html += `<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; ">`;

                // Iterate over candidates and generate grid items
                item.candidates.forEach(candidate => {
                    html += `<div style="text-align: center; padding: 10px; border: 1px solid #ccc; border-radius: 20px;">`;
                    html += `<img src="${candidate.candidatePhoto}" alt="${candidate.candidateName}" style="max-width: 100%; height: auto; border-radius: 20px" />`;
                    html += `<p>${candidate.candidateName}</p>`;
                    html += `<p>Votes: ${candidate.voteCount}</p>`;
                    html += `</div>`;
                });

                html += `</div>`; // Close candidate-grid

                html += `</div>`; // Close position-container
            });
        ...
        const { data, error } = await supabaseAdmin.from("created_candidates_tb").select("*").eq("classification", classification) as { data: CandidatesDB[] | null, error: PostgrestError | null };

        const myArray: PrintingType[] | undefined = customCandidateArray(data);
        ...
        try {
            ...
            await page.setContent(generateHTMLTemplate(myArray));
            await page.pdf({ path: pdfPath, format: 'A4' });
            await browser.close();


            const staticPdfPath = path.join(process.cwd(), 'static', 'ssg_latest_vote_counts.pdf');

            // Check if the file already exists in the 'static' directory
            if (await fs.pathExists(staticPdfPath)) {
                // Delete the existing file before moving the new PDF file
                await fs.remove(staticPdfPath);
            }
            // Move the generated PDF file to the 'static' directory
            await fs.move(pdfPath, staticPdfPath);

            return fail(200, { msg: "Success", link: `${url.origin}/ssg_latest_vote_counts.pdf` });

        } catch (error) {
            console.log(error)
            return fail(400, { msg: "Error Downloading" });
        }
